...
export class FolderScope {
  ...
  async initialize(): Promise<void> {
    ...
    if (!session) {
      console.warn('  ⚠  FolderScope: No vault session, folder scoping disabled');
      return;
    }
    ...
    try {
      ...
      const folders: Array<{ id: string; name: string }> = JSON.parse(foldersResult.stdout.toString());
      const items: Array<{ name: string; folderId: string | null }> = JSON.parse(itemsResult.stdout.toString());
      ...
      for (const [clientId, folderNames] of this.clientFolderNames) {
        ...
        for (const name of folderNames) {
          ...
          if (id) {
            ...
            console.warn(`  ⚠  FolderScope: Folder "${name}" not found in vault (client: ${clientId})`);
          }
        }
        if (folderIds.size > 0) {
      ...
      // Log summary
      for (const [clientId, folderNames] of this.clientFolderNames) {
        const resolvedCount = this.clientAllowedFolderIds.get(clientId)?.size || 0;
        console.log(`  ✓ Folder Scope: ${clientId} → ${folderNames.join(', ')} (${resolvedCount} resolved)`);
      }
    } catch (error) {
      console.warn(`  ⚠  FolderScope: Init failed — ${error instanceof Error ? error.message : error}`);
    }
  }
  ...
  isRestricted(clientId: string): boolean {
    ...
    return this.clientAllowedFolderIds.has(clientId);
  }
  ...
  isAllowed(clientId: string, itemName: string): boolean {
    ...
    const allowedFolders = this.clientAllowedFolderIds.get(clientId);
